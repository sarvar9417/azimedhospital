    ...
    const getRoom = useCallback(async () => {
        try {
            ...
            if (data.position === "band") {
                ...
                console.log(Math.abs((new Date(new Date(data.endDay).getFullYear(), new Date(data.endDay).getMonth(), new Date(data.endDay).getDate()) - new Date(new Date(data.beginDay).getFullYear(), new Date(data.beginDay).getMonth(), new Date(data.beginDay).getDate())) / oneDay) + 1)
            }
            setRoom(data)
        } catch (e) {
            notify(e)
                            ...
                            <div className="invoice-from">
                                <table className='table table-bordered' style={{ fontSize: "10pt", fontFamily: "times" }}>
                                    <thead >
                                        ...
                                        <tr>
                                            <td className='w-25 p-1 text-start px-3'>Shifokor</td>
                                            <td className='w-75 p-1 px-3'>{connector && connector.doctor} </td>
                                        </tr>
                                        <tr>
