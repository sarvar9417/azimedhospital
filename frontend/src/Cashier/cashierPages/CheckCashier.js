...
export const CheckCashier = () => {
  ...
  const setAllPayment = useCallback(
    (event) => {
      let s = [...sections]
      s.map((section, key) => {
  ...
  const getSections = useCallback(async () => {
    try {
      const fetch = await request(
        `/api/section/cashierconnector/${connectorId}`,
        'GET',
        null,
        {
          Authorization: `Bearer ${auth.token}`,
  ...
  const getSections1 = useCallback(async () => {
    try {
      ...
      const data = await request(
        `/api/service/cashierconnector/${connectorId}`,
        'GET',
        null,
        {
          Authorization: `Bearer ${auth.token}`,
  ...
  const inputPriceSection = useCallback(
    (event, key) => {
      ...
      if (parseInt(event.target.value) === sections[key].price) {
        setSections(
          Object.values({
            ...
            [key]: {
              ...sections[key],
              priceCashier: parseInt(event.target.value),
              payment: "to'langan",
              commentCashier: ' ',
            },
          }),
  ...
  const inputPriceService = useCallback(
    (event, key) => {
  ...
  const checkboxSection = useCallback(
    (event, key) => {
      if (event.target.checked) {
        setSections(
          Object.values({
            ...
            [key]: {
              ...sections[key],
              priceCashier: sections[key].price,
              payment: "to'langan",
            },
          }),
        )
  ...
  const checkboxService = useCallback(
    (event, key) => {
      if (event.target.checked) {
        setServices(
          Object.values({
            ...
            [key]: {
              ...services[key],
              priceCashier: services[key].price,
              payment: "to'langan",
            },
          }),
        )
  ...
  const checkPrices = () => {
    ...
    sections &&
      sections.map((section) => {
        if (
          ...
          section.commentCashier.length < 6
        ) {
          k++
          return notify(
            "Iltimos mijoz to'lovni bajarolmagani sababini to'liq ko'rsating.",
          )
        }
      })
    services &&
      services.map((service) => {
        if (
          ...
          service.commentCashier.length < 6
        ) {
          k++
          return notify(
            "Iltimos mijoz to'lovni bajarolmagani sababini to'liq ko'rsating.",
    ...
    if (
      payment.total !== payment.cash + payment.card + payment.transfer ||
      payment.total !== bepaid
    ) {
      return notify(
        "Diqqat to'lov turida summani kiritishda xatolikka yo'l qo'ydingiz. Iltimos to'lov turidagi summalarni yana bir bor tekshiring",
      )
    }
    if (payment.type === '') {
  ...
  const getchangeSections = useCallback(
    async (event) => {
      try {
        const fetch = await request(
          `/api/clients/cashierid/${parseInt(event.target.value)}`,
          'GET',
          null,
          {
            Authorization: `Bearer ${auth.token}`,
  ...
  const DeleteSection = useCallback(async () => {
    try {
      const fetch = await request(
        `/api/section/${delSection && delSection._id}`,
        'DELETE',
        null,
        {
          Authorization: `Bearer ${auth.token}`,
  ...
  const DeleteService = useCallback(async () => {
    try {
      const fetch = await request(
        `/api/service/${delService && delService._id}`,
        'DELETE',
        null,
        {
          Authorization: `Bearer ${auth.token}`,
        ...
        <table className="w-100 mt-3">
          <thead>
            <tr style={{ borderBottom: '1px solid #999' }}>
              ...
              <th
                style={{ width: '35%', textAlign: 'center', padding: '10px 0' }}
              >
                Bo'limlar
              </th>
              <th
                ...
                return (
                  <tr>
                    ...
                    <td style={{ width: '25%', padding: '10px 0' }}>
                      <input
                        onChange={(event) => inputPriceSection(event, key)}
                        value={section.priceCashier}
                        type="number"
                      ...
                      <input
                        disabled={loading}
                        checked={
                          section.priceCashier === section.price ? true : false
                        }
                        id={`checkbox${key}`}
                        onChange={(event) => checkboxSection(event, key)}
                        type="checkbox"
                        className="check"
                    ...
                    >
                      {section.price !== section.priceCashier ? (
                        <textarea
                          value={section.commentCashier}
                          onChange={(event) => inputCommentSection(event, key)}
                          key={key}
                          placeholder="To'lov bajarilmagan holatda sababini ko'rsating"
              ...
              services.map((service, key) => {
                ...
                return (
                  <tr>
                    ...
                    <td style={{ width: '25%', padding: '10px 0' }}>
                      <input
                        onChange={(event) => inputPriceService(event, key)}
                        value={service.priceCashier}
                        type="number"
                        className="form-control"
                        style={{
                          width: '80%',
                      ...
                      <input
                        disabled={loading}
                        checked={
                          service.priceCashier === service.price ? true : false
                        }
                        id={`checkboxservice${key}`}
                        onChange={(event) => checkboxService(event, key)}
                        type="checkbox"
                        className="check"
                        style={{ position: 'absolute' }}
                      />
                    ...
                    >
                      {service.price !== service.priceCashier ? (
                        <textarea
                          value={service.commentCashier}
                          onChange={(event) => inputCommentService(event, key)}
                          key={key}
                          placeholder="To'lov bajarilmagan holatda sababini ko'rsating"
                          className="addDirection"
                          minLength="6"
          ...
          <div className="card">
            ...
            <div className="card-body">
              ...
              <div className="row ms-3 me-5">
                ...
                <div className="col-6">
                  <div className="fw-bold  text-end text-danger">
                    {allPrice - (payment.total + oldPayments)}
                  </div>
                </div>
                <hr />
              </div>
